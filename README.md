...
[![Version](https://img.shields.io/badge/version-2.2.0-green.svg)](docs/CHANGELOG.md)
...
4.  **Export Tool Manifest**: Run `tb.run("plugin_export_manifest")` to write `Saved/UEFN_Toolbelt/tool_manifest.json` — a machine-readable index of all 355 tools with their parameter signatures, types, defaults, and categories. An AI agent can load this file and immediately know what every tool does and how to call it, without reading source code.
...
> **355 tools · 54 categories** — all return `{"status": "ok"/"error", ...}` structured dicts.
...
|:---|:---|:---|
| **Tool Layer** | `Saved/UEFN_Toolbelt/tool_manifest.json` | 355 tools — names, params, types, defaults |
...
- **Structured Returns Everywhere (Phase 21 complete)**: All 355 tools return `{"status": "ok"/"error", ...}` dicts. Zero `None` returns remain. MCP callers (Claude Code, `client.py`, scripts) can read every result programmatically — no log parsing required.
