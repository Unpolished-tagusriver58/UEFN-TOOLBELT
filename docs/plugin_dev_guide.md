...
```python
# At the top of your plugin, after other imports:
try:
    from UEFN_Toolbelt.dashboard_pyside6 import _QSS as _DASH_QSS
except Exception:
    _DASH_QSS = (          # minimal inline fallback
        ...
        "QPushButton:pressed{background:#3A3AFF;border-color:#3A3AFF;}"
        "QLineEdit{background:#212121;border:1px solid #363636;color:#CCCCCC;"
        "padding:3px 7px;border-radius:3px;min-height:24px;}"
...
class MyPluginWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setStyleSheet(_DASH_QSS)   # ← always, always, always
        ...
```

And **always** drive Qt's event loop from Unreal's Slate tick — without it the window
will be invisible:
...
def _tick(dt):
    try:
        if not win.isVisible():
            unreal.unregister_slate_post_tick_callback(tick_handle[0]); return
        app.processEvents()
    except Exception:
        try: unreal.unregister_slate_post_tick_callback(tick_handle[0])
