...
It runs inside the editor and exposes 161 tools through:
...
- Run any of the 161 registered tools by name
...
| `plugin_export_manifest` | — | Export `tool_manifest.json` — machine-readable index of all 161 tools with full parameter signatures (name, type, required, default) for AI-agent and automation use |
...
| `run_tool` | `tool_name`, `kwargs={}` | Run any of the 161 registered tools |
